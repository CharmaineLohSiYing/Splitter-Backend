...
const getLoansGroupedByFriendsService = async (userId) => {
  ...
  for (transaction of transactions) {
    ...
    if (friendIsPayer) {
      debts[mobileNumberDirectory[friendUserId]].debt =
        debts[mobileNumberDirectory[friendUserId]].debt + transaction.amount;
    } else {
      debts[mobileNumberDirectory[friendUserId]].debt =
        debts[mobileNumberDirectory[friendUserId]].debt - transaction.amount;
    }
...
const getLoansByFriendMobileNumberService = async (
  userId,
  friendMobileNumber
) => {
  let retrievedFriend;
  ...
  if (!retrievedUser) {
    try {
      retrievedFriend = (await createUnregisteredUser(friendMobileNumber))._id;
    } catch (err) {}
  } else {
